...
// GeneralsMD tree's copy of this header. Only MSG_GX_CHEAT_GIVE_MONEY carries an explicit
// "= 1950"; everything after it runs on implicitly, so inserting an enumerator anywhere in the
// block would silently renumber the tail. Assert the whole range so that mistake is a build error
// naming the exact value that moved, rather than a desync found on a LAN evening.
